...
void CMfcStartDlg::OnBnClickedBtnDraw()
{
	...
	DrawCircle(pFm, m_nptXStart, m_nptYStart, m_nRadius, RGB(255, 80, 0));
...
void CMfcStartDlg::MoveCircle(bool bInit, int nMax)
{
	...
	DrawCircle(pFm, dSttX += dXAdd, dSttY += dYAdd, m_nRadius, RGB(255, 80, 0));
...
void CMfcStartDlg::UpdateDisplay(CDC* pDC)
{
	...
	if (m_ptLoadImgCenter.x != -1 && m_ptLoadImgCenter.y != -1) {
		pPicDC->SetPixel(m_ptLoadImgCenter, RGB(255, 0, 0));
...
void CMfcStartDlg::ImgOpenProc(CString strPathName)
{
	if (m_bThreadEnable) {
		...
		int nPitch = abs(imgLoad.GetPitch());
		if (imgLoad.GetPitch() < 0) {
			for (int i = 0; i < m_ImgDisp.GetHeight(); i++) {
				memcpy(pImgDest + (nPitch * i), pImgSrc - (nPitch * i), nPitch);;
			}
		}
		else {
			memcpy(pImgDest, pImgSrc, nPitch * m_ImgDisp.GetHeight());
		}
...
void CMfcStartDlg::FindCircleCenter()
{
	...
	for (int y = 0; y < m_ImgDisp.GetHeight(); y++) {
		for (int x = 0; x < m_ImgDisp.GetWidth(); x++) {
			...
			BYTE bGray = (GetRValue(temp_color) * 30
				+ GetGValue(temp_color) * 59
				+ GetBValue(temp_color) * 11) / 100;

			BYTE bInvertbGray = 255 - bGray;

			pFm[y * nPitch + x] = bInvertbGray > 30 ? 255 : 0;		// 반전 시킨 값을 30 을 기준으로 흑백으로 구분
	...
	if (m_bThreadEnable) {
		m_ptLoadImgCenter.x = nMinX + roundf((float)abs(nMaxX - nMinX) / 2);
		m_ptLoadImgCenter.y = nMinY + roundf((float)abs(nMaxY - nMinY) / 2);

		m_nRadius = (int)roundf((float)abs(nMaxX - nMinX) / 2);
